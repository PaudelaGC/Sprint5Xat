    ...
    if (action === 'signup') {
      ...
      try {
        socket.emit('check_username', { username }, (response: boolean) => {
          ...
          socket.emit(
            'check_password',
            { username, password, purpose: 'login' },
      ...
      socket.emit('check_username', { username }, (response: boolean) => {
        ...
        socket.emit(
          'check_password',
          { username, password, purpose: 'delete' },
          ({ success, reason }: { success: boolean; reason?: string }) => {
            if (success) {
  ...
  return (
    <div>
      ...
      <div className="centered-form">
        <div className="yellow-block">
          ...
          {confirmDelete ? (
            <div>
              ...
              <label>
                Password:
                <input className="input-text" type="password" value={deletePassword} onChange={(e) => setDeletePassword(e.target.value)} />
              </label>
            ...
            <form onSubmit={handleAction}>
              <label>
                Username:
                <input className="input-text" type="text" name="username" value={username} onChange={handleInputChange} />
              </label>
              <br />
              <label>
                Password:
                <input className="input-text" type="password" name="password" value={password} onChange={handleInputChange} />
              </label>
              <br />
              <button className="button" type="submit" onClick={() => setAction('signup')}>
                Sign Up
              </button>
              <button className="button" type="submit" onClick={() => setAction('login')}>
                Log In
              </button>
        ...
        <div className="connected-users yellow-block" style={{ marginTop: '20px' }}>
